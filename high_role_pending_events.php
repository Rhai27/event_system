...
if (isset($_POST['cancel_event'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE qevents SET qevent_status = 'Declined' WHERE event_id = ?");
...
if (isset($_POST['accept_event'])) {
    ...
    try {
        // Update qevents table to set qevent_status to "Confirmed"
        $stmt = $pdo->prepare("UPDATE qevents SET qevent_status = 'Confirmed' WHERE event_id = ?");
        $stmt->execute([$eventId]);

        // Redirect to the high_role_dashboard after updating event status
        ...
        <?php
            try {
                ...
                foreach ($result as $row) {
                    ...
                    $editButton = "<a href='other_high_role_edit_event.php?id={$row['event_id']}&username={$row['username']}' class='edit-button'>View</a>";
                    
                    if ($row['qevent_status'] === 'Pending') {
                        $viewButton = "<form method='post'>
                                        <input type='hidden' name='event_id' value='{$row['event_id']}'>
                                        <button type='submit' name='accept_event' class='edit-button' onclick='return confirm(\"Are you sure you want to accept this event?\")'>Accept Event</button>
                                    </form>";
                        
                        $cancelButton = "<form method='post'>
                                        <input type='hidden' name='event_id' value='{$row['event_id']}'>
                                        <button type='submit' name='cancel_event' class='edit-button' onclick='return confirm(\"Are you sure you want to decline this event?\")'>Decline Event</button>
                                    </form>";
                    }
                    ...
                    echo "<tr>
                            ...
                            <span><td>$editButton $viewButton $cancelButton</td></span>
                        </tr>";
                }
